...
def convert_smshg_yaml_to_json(template_path, example_json_path, smshg_files_dir):
    ...
    # SMSHGのテンプレート情報を取得
    smshg_template = templates['gun_types']['super_large_caliber_gun']

    # SMSHGファイルを処理
    for i in range(1, 2): # SMSHG001.ymlのみ
        smshg_file_name = f"SMSHG{i:03d}.yml"
        smshg_file_path = os.path.join(smshg_files_dir, smshg_file_name)
        
        if not os.path.exists(smshg_file_path):
            print(f"ファイルが見つかりません: {smshg_file_path}")
            continue

        with open(smshg_file_path, 'r', encoding='utf-8') as f:
            smshg_data = yaml.safe_load(f)

        # 新しいJSON構造を構築
        new_json_data = {
            "equipment_type": smshg_template['display_name'],
            "common": {},
            "specific": {},
        ...
        new_json_data['common']['名前'] = smshg_data.get('name')
        new_json_data['common']['ID'] = smshg_data.get('id')
        new_json_data['common']['重量'] = smshg_data.get('weight')
        new_json_data['common']['人員'] = smshg_data.get('personnel')
        new_json_data['common']['開発年'] = smshg_data.get('developed_year')
        new_json_data['common']['開発国'] = smshg_data.get('country')
        
        resources = smshg_data.get('resources', {})
        ...
        specific_elements = smshg_data.get('specific_elements', {})
        ...
        # JSONファイルとして保存
        json_output_path = smshg_file_path.replace('.yml', '.json')
        with open(json_output_path, 'w', encoding='utf-8') as f:
            json.dump(new_json_data, f, ensure_ascii=False, indent=2)
        print(f"変換完了: {smshg_file_path} -> {json_output_path}")
...
if __name__ == "__main__":
    ...
    smshg_directory = os.path.join(base_dir, "equipments/SMSHG")

    convert_smshg_yaml_to_json(template_file, example_json_file, smshg_directory)
